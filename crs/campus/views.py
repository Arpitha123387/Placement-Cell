 ...
 if  request.user.is_authenticated and request.user.groups.filter(name='student').exists():
    if request.method == "POST":
            ...
            if form.is_valid():
                ...
                cb=request.POST.get('cgpa_Bachelors')
...
def jd(request):
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method == "POST":
                    ...
                    book.marketing=request.POST.get("marketing")
                    book.engineer=request.POST.get("engineer")
                    book.save()
...
def applyjob(request):
    if request.user.is_authenticated and request.user.groups.filter(name='student').exists():
      ...
      if request.method == "POST":
        ...
        if (b == "mk"):
            y = job_pos.objects.filter(salary__gte=sal, bond_years__lte=bon,marketing="yes").order_by('salary')
        if (b == "er"):
            y = job_pos.objects.filter(salary__gte=sal, bond_years__lte=bon,engineer="yes").order_by('salary')
        print(y)
        if(len(y)==0):
...
def selectstu(request):
    ...
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method == "POST":
             ...
             Bachelors=request.POST.get("Bachelors")
             ...
             for i in x:
                 b=stu_details.objects.filter(class_10_cgpa__gte=tenth,class_12_percentage__gte=twth,cgpa_Bachelors__gte=Bachelors,username=i['student_id'])
                 if(b.count()>0):
                   y.append(b)
             print("the total number is",len(y))
             if(len(y)==0):
